        ...
        <>
            {cards.map((card, index)=>{ 
                return <div key = {index} className="card" data-ticker={card.ticker} onClick={(e) => handleCardClick(e)}>
                        ...
                        <p data-ticker={card.ticker} onClick={(e) => handleCardClick(e)}>{card.count} people mentioned this. <Link to={`/main/${card.ticker}`} transition="glide-left" className="orange">
                            details
                        </Link></p> 
                </div> 
            })}
